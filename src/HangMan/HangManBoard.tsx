...
function HangManBoard() {
  ...
  return (
    <div>
      {incorrectGuesses >= 6 && <div className={"text-danger"}>You loose. Word was: <b>{word}</b></div>}
      ...
      <div className={"container-sm d-flex gap-2 align-items-center justify-content-center"}>
        ...
        {wordPuzzle.map((char, index) => (
          <div className={"text-decoration-underline"} key={index}>{char}</div>
        ))}
      </div>
